...
from bel_json_importer.constants import bel_func_short
from bel_json_importer.n4j_importer import Neo4jImporter
from bel_json_importer.n4j_meta import Neo4jClient, Edge, Node
from bel_json_importer.n4j_structure import ABSTRACT_LABELS
...
class Neo4jBel:
    ...
    def import_json(
        ...
        """Import BEL JSON file(s) into Neo4J.
    ...
    def __add_missing_edges(self, from_class, to_class, edge_name, bel_function) -> int:
        ...
        for entry in tqdm(results, desc=f"Adding {edge_name} edges"):
            ...
            subj_node = Node(
                labels=from_class,
                props={"namespace": ns, "name": name, "pure": True, "bel": bel},
            )
            subj_node_id = self.client.merge_node(node=subj_node)

            self.client.merge_edge_by_node_ids(
                subj_id=subj_node_id, rel=Edge(labels=edge_name), obj_id=entry["obj_id"]
            )
            added += 1
    ...
    def _create_pure_nodes_to_modified(self) -> int:
        """Create all has_modified_(protein|gene) edges in Neo4J (proteins without a pure counterpart)."""
        edge_classes = {
...
type(r) as edge_class"""
        ...
        for row in tqdm(results, desc="Add edges to pure nodes"):
            ...
            node_class = [x for x in node_classes if x not in ABSTRACT_LABELS][
                0
            ]  # Should only have 1 non-abstract
...
if __name__ == "__main__":
    ...
    b.import_json(
        input_path="F:\\scai_git\\bms\\parkinsons.bel.json",
    )
