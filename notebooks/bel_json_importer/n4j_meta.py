...
class Neo4jClient:
    ...
    def execute(self, cypher: str):
        ...
        return self.session.run(cypher).data()  # type: ignore
    ...
    def create_node(self, node: Node) -> int:
        ...
        cypher = f"CREATE (n:{node.cypher_labels} {node.cypher_props}) return elementId(n) as node_id"
    ...
    def create_edge(self, subj: Node, edge: Edge, obj: Node) -> Relationship:
        ...
        r = self.session.run(cypher).values()[0]  # type: ignore
        return Relationship(*r)

    def merge_node(self, node: Node):
        """Create a node with given props if it does not exist."""
        cypher = f"""MERGE (n:{node.cypher_labels} {node.cypher_props}) return elementId(n) as id"""
    ...
    def merge_edge(self, subj: Node, rel: Edge, obj: Node):
        ...
        return self.session.run(cypher)  # type: ignore
    ...
    def delete_everything(
        ...
        self.session.run(cypher_edges)  # type: ignore
        self.session.run(cypher_nodes)  # type: ignore
    ...
    def relationship_types(self) -> list[str]:
        ...
        """
        return [
            x["relationshipType"] for x in self.execute("CALL db.relationshipTypes")
        ]
