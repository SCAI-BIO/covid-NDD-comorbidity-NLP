...
from bel_json_importer.constants import normalized_pmod, bel_func_short, NODES, EDGES
from bel_json_importer.n4j_structure import node_map, edge_map
from bel_json_importer.n4j_meta import Neo4jClient, Node, Edge
...
class Neo4jImporter:
    ...
    def __init__(self, file_path: Path, n4j_client: Neo4jClient):
    ...
    def insert(self) -> tuple[bool, int]:
        ...
        add_edges = self.insert_statements_and_sets(
            stmts_and_sets["statements_and_sets"]
        )
        ...
        for e in tqdm(statements_and_sets, desc="Insert BEL Statements"):
            ...
            if dtype == "sets":
                ...
                if len(data) > 1 and "object" in data[2]:
                    ...
                    new_edges += self.insert_bel_edge(
                        annotation,
                        citation,
                        evidence,
                        pmid,
                        neo4j_relation_class,
                        subject_id,
                        object_id,
                    )
    ...
    def format_prop(prop_name: str, prop: dict) -> dict:
        ...
        if prop:
            for key, value in prop.items():
                suffix = "".join(
                    [stem.capitalize() for stem in key.split("_")]
                )  # foo_bar -> FooBar
    ...
    def insert_bel_edge(
        ...
        if not edge_exists:
            ...
            record = self.client.merge_edge_by_node_ids(
                subj_id=subject_id, rel=new_edge, obj_id=object_id
            )
...
WHERE elementId(startNode(r)) = "{node_id}" AND elementId(endNode(r)) = "{child_node_id}" RETURN r"""
            ...
            if not exists:
                ...
                self.client.merge_edge_by_node_ids(
                    subj_id=node_id, rel=new_edge, obj_id=child_node_id
                )
